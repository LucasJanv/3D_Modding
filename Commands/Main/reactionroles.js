            ...
            reactionMessage.awaitReactions(Filter, {max: 1, time: 30000, errors: ["time"]}).then(collected => {
                ...
                switch (reaction.emoji.name) {
                    case "📢":
                        ...
                        if (message.member.roles.cache.has(Role1.id)) {return message.channel.send(`You already have this role.`).then(msg => msg.delete({timeout: 1000 * 5}))}; 
                        // Adding the role.
                        message.member.roles.add(Role1).then(message.channel.send(`${Role1.name} Role added!`)).then(msg => msg.delete({timeout: 1000 * 5}));
                ...
                switch (reaction.emoji.name) {
                    case "✔️":
                        ...
                        if (message.member.roles.cache.has(Role1.id)) {return message.channel.send(`You already have this role.`).then(msg => msg.delete({timeout: 1000 * 5}))};
                        // Adding the role.
                        message.member.roles.add(Role2).then(message.channel.send(`Role added!`)).then(msg => msg.delete({timeout: 1000 * 5}));
                        // Breaking the switch statement to make sure no other cases are executed.
                        break
                }
            })
